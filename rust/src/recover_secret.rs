...
pub fn recover_secret(triplets: Vec<[char; 3]>) -> String {
    // collect all symbols and build a precedence relation from input triplets

    let mut symbols = HashSet::new();
    let mut prec: HashMap<char, HashSet<char>> = HashMap::new();

    for t @ [x, y, z] in triplets {
        symbols.extend(t);
        prec.entry(x).or_default().extend([y, z]);
        prec.entry(y).or_default().insert(z);
        prec.entry(z).or_default();
    }

    // find (reversed) topological order of all the symbols given their precedence

    fn topsort(
        symbol: char,
        pos: &mut usize,
        secret: &mut [char],
        prec: &mut HashMap<char, HashSet<char>>,
    ) {
        // marks symbol as visited by removing it from the precedence table
        for s in prec.remove(&symbol).unwrap() {
            if prec.contains_key(&s) {
                topsort(s, pos, secret, prec);
            }
        }
        *pos -= 1;
        secret[*pos] = symbol;
    ...
    let mut secret = vec![char::default(); symbols.len()];

    for symbol in symbols {
        if prec.contains_key(&symbol) {
            topsort(symbol, &mut pos, &mut secret, &mut prec);
        }
    }

    // decode secret message from the reversed topological order
...
mod tests {
    ...
    fn help() {
        assert_eq!(
            recover_secret(vec![['h', 'l', 'p'], ['e', 'l', 'p'], ['h', 'e', 'l']]),
            "help"
        );
    }
    ...
    fn alphabet() {
        #[rustfmt::skip]
        mod data {
            pub(crate) const TRIPLETS: [[char; 3]; 474] = [
        ...
        assert_eq!(
            recover_secret(data::TRIPLETS.into()),
